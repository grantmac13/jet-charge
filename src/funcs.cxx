...
bool dropLowStatEvent(int ev, std::vector<int> drop_event_n)
{
	bool dropped = false;
	for(int j = 0; j < drop_event_n.size(); j++){
		if(drop_event_n[j] == ev){dropped = true;}
	}
//	if(dropped){std::cout << "event dropped!\n";}
	return dropped;
}
