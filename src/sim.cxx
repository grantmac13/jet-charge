...
int main(int argc, const char **argv){
	...
	TFile *p6match = new TFile(("./out/sim/hists/p6_hists_det_resolut_R" + (string) argv[3] + ".root").c_str(),"READ");
...
//	eventTree->Branch("g_mcd",&g_mcd);
	...
	vector<double> part_jetmass;
	vector<double> det_jetmass;

	vector<double> miss_jetmass;
	vector<double> fake_jetmass;
	...
	TH1D* jetpt_match = new TH1D("jetpt_match", "", 11, 5, 60);
	TH1D* jetpt_miss = new TH1D("jetpt_miss", "", 11, 5, 60);
	TH1D* jetpt_fake = new TH1D("jetpt_fake", "", 11, 5, 60);


	//cout << "\nDEBUG: histograms being made???\n";

	TH1D* pt_part = new TH1D( "pt_part", "", 15, 5, 80);
	TH1D* pt_det = new TH1D( "pt_det", "", 15, 5, 80);
...
*/
    ...
    //if(!pt_res_py){nSources = 1;} // if resolution doesn't exist, don't do systematic loop
    for(int iSyst = 0; iSyst < nSources; iSyst++){
        ...
        if(iSyst == 6){cout << "RUNNING WITH SMEARED GENERATOR ~CHARGE~ SPECTRUM!" << endl << endl;}
...
//	    InitReader(GEANTReader, GEANTChain, nEvents, "All", det_absMaxVz, det_vZDiff, det_evPtMax, det_evEtMax, det_evEtMin, det_DCA, det_NFitPts, det_FitOverMaxPts, sim_maxEtTow, hc, false, det_badTowers, dat_bad_run_list);
            ...
            if(match){
                if(iSyst == 0){
                    ...
                    for(int kj = 0; kj < p_Jets.size(); kj++){
                        pt_part->Fill( p_Jets[kj].pt(), mc_weight );
                    ...
                    for(int kj = 0; kj < g_Jets.size(); kj++){
                        pt_det->Fill( g_Jets[kj].pt(), mc_weight );
...
//		    }
                ...
                if(iSyst == 0 && ( p_Jets.size() != 0 || g_Jets.size() != 0 ) ){
                    eventTree->Fill();
                    jetChargeTree->Fill();
                }
                else if(iSyst == 1 && p_Jets.size() != 0){
        ...
        //cout << "number of jets in systematic = " << njet_events[iSyst] << "\n";
    } // systematic loop
    ...
    if(match){
        ...
        //cout << "writing out trees and pt spectra...\n";

        pt_part->Write();
        pt_det->Write();

//        cout << "writing new pt spectra such as:" << pt_part->GetName() << "\n";
...
/////////////////////////////////////////
	...
	cout << "wrote jets containing only constituents with pT > " << partMinPt << endl;
