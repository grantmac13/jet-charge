...
/*
// not currently used, not sure how useful splitting out as a function this is...
// calculate the total systematic error of histogram
void calcAbsErr(TH1D* err, vector<TH1D*> systs){
	for(int i = 1; i < err->GetNbinsX(); i++){
		double quad2 = 0.0;

		for(int j = 0; j < (int) systs.size(); j++){
			quad2 += systs[j]->GetBinContent(i) * systs[j]->GetBinContent(i);
		}
		err->SetBinError( i, sqrt( quad2 ) );
	}
}
*/
...
void plot_Qjet_unfold(double R = 0.4, double k = 0.0){
...
//cout << "distributions loaded; starting systematics\n";
		for(int k = 0; k < nsyst; k++){
			systs[j][k] = (TH1D*) unfold->Get( Form( ("unfold_" + syst_names[k] + "%1.0f%1.0f").c_str(), jetPtLo[j], jetPtHi[j]  ) );
//cout << Form( ("unfold_" + syst_names[k] + "%1.0f%1.0f").c_str(), jetPtLo[j], jetPtHi[j]  ) << "\n";
		}
...
//cout << "finished systematics loops\n";
...
//		data_2Ds[j]->Scale(1.0/(d_jets->Integral( (int) jetPtLo[j] + 1, (int) jetPtHi[j] )  ) );
		...
		/*
		OLD: match jets between particle, detector level and fill n-th jet pt bin if detector level jet is within range and normalize both particle and detector level by this number of detector level jets
		NEW: match jets between particle, detector level and fill n-th jet pt bin based on respective level's jet pt and normalize by these DIFFERENT number of jets
		*/


		// when matched jets, use above. when non-matched jets, use below
//		jetQ_p6_p[j]->Scale(1.0/(p6_jets_p->Integral( (int) jetPtLo[j] + 1, (int) jetPtHi[j] )  ) );
		...
		jetQ_data[j]->SetMarkerColor(kViolet-3);
		...
		jetQ_data[j]->SetMarkerStyle(kOpenSquare);
...
//		leg[j]->AddEntry(jetQ_data[j], "Data", "p");
//		leg[j]->AddEntry((TObject*)0, Form("mean: %1.3f", jetQ_data[j]->GetMean()), "");
...
/*
		if(kappa == "05"){
			jetQ_data[j]->SetXTitle(" < Q_{jet} >  (#kappa = 0.5)");
		}
		else if(kappa == "00"){
			jetQ_data[j]->SetXTitle(" < Q_{jet} >  (#kappa = 0.0)");
		}
		else if(kappa == "03"){
			jetQ_data[j]->SetXTitle(" < Q_{jet} >  (#kappa = 0.3)");
		}
		else if(kappa == "07"){
			jetQ_data[j]->SetXTitle(" < Q_{jet} >  (#kappa = 0.7)");
		}
*/

		jetQ_err[j]->Draw("E2");
		jetQ_unfold[j]->Draw("p same");
//		jetQ_data[j]->Draw("p same");
...
//		drawText("p+p #sqrt{s_{NN}} = 200 GeV}", .25, .85, 15);
		...
		c[j]->SaveAs(Form("./plots/unfoldedJetQ_jetpt_%1.0f_%1.0f_k" + kappa + "_R" + rad + ".png", jetPtLo[j], jetPtHi[j]), "RECREATE");
	}
}
